 ...
 *
 */
public class AveragingEnergyCorrected2 extends JFrame implements XYToolTipGenerator{
	...
	public AveragingEnergyCorrected2() {
		...
		double rav = 1.0;
//		// eq for av3
//		double f_ortho = rav*-0.346096 + 1.126528;
		// eq for av
		double f_ortho = rav*-0.312370 + 1.05062;
		double f_corr = 1.0;
...
//		r.setSeriesLinesVisible(i++, true);
		...
		for(String name : names){
			...
			double []sigmaDelta = new double[sigma.length];
			double []sigmaDeltaAvg = new double[sigma.length];
			...
			for (int i = 0; i < sigmaAvg.length; i++) {
				...
				sigmaDelta[i] = sigmai*f_ortho - sigmaAvgi;
			}
			
			SigmaProfileGenerator2.averageCharges(rav, sigmaDelta, sigmaDeltaAvg, area, sparser.x, sparser.y, sparser.z);

			for (int i = 0; i < sigmaAvg.length; i++) {
				double sigmai = sigma[i];
				double sigmaAvgi = sigmaAvg[i];
				double sigmaAvgCorri = (sigmaAvgi + f_corr * sigmaDeltaAvg[i])/f_ortho;
